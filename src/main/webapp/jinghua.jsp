...
<head>
    ...
    <link href="css/prestoge.css" rel="stylesheet">
    <!-- Bootstrap -->
...
<div class="container-fluid">
                    ...
                    <tr>
                        <td>
                            <c:if test="${item.main_recommend eq 1 }">
                                <a href="secondContent?mainId=${item.main_id}"><img src="image/pin_1.gif"
                                                                                    id="${item.main_id}img"/>[日月精华]&nbsp;&nbsp; ${item.main_title }
                                </a>
                                <c:if test="${sessionScope.loginFlag}">
                                    <a style="color: green" href="javascript:void(0)"
                                       onclick="setjinghua('${item.main_id}','qx')">[取消精华]</a>
                                </c:if>

                            </c:if>
                            <c:if test="${item.main_recommend ne 1 }">
                                <a href="secondContent?mainId=${item.main_id}"><img src="image/folder_new.gif"/>[最新帖子]&nbsp;&nbsp; ${item.main_title }
                                </a>
                                <c:if test="${sessionScope.loginFlag}">
                                    <a style="color: red" href="javascript:void(0)"
                                       onclick="setjinghua('${item.main_id}','sz')">[设置精华]</a>
                                </c:if>
                            </c:if>
    ...
    <!-- 配置文件 -->
    <script type="text/javascript" src="ueditor/js/ueditor.config.js"></script>
    <!-- 编辑器源码文件 -->
    <script type="text/javascript" src="ueditor/js/ueditor.all.js"></script>
    <!-- 实例化编辑器 -->
